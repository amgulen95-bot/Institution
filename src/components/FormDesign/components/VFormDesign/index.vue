<template>
  <Layout class="bg-white">
    <LayoutSider :class="`left ${prefixCls}-sider`" collapsible collapsedWidth="0" width="270" :zeroWidthTriggerStyle="{
      'margin-top': '-64px',
    ...
    <LayoutContent>
      <Toolbar @handle-open-json-modal="handleOpenModal(jsonModal!)"
        ...
        @handle-open-code-modal="handleOpenModal(codeModal!)" @handle-clear-form-items="handleClearFormItems" />
      <FormComponentPanel :current-item="formConfig.currentItem" :data="formConfig"
        @handle-set-select-item="handleSetSelectItem" />
    </LayoutContent>
    <LayoutSider :class="`right ${prefixCls}-sider`" collapsible :reverseArrow="true" collapsedWidth="0" width="270"
      :zeroWidthTriggerStyle="{ 'margin-top': '-64px', 'background-color': 'gray' }" breakpoint="lg">
...
watch(() => props.form, () => {
  setFormConfig(props.form as IFormConfig)
}, { deep: true })
...
const getFormConfigContent = () => {
  return cloneDeep(formConfig.value)
}

defineExpose({ getFormConfigContent })
