...
export const useAppStore = defineStore({
  ...
  getters: {
    ...
    getWindowAvailableHeight() {
      return this.windowAvailableHeight
    }
  },
  actions: {
    setWindowAvailableHeight(): void {
      const sideBarDom = document.getElementsByClassName('vben-layout-sideBar')[0]
      if (sideBarDom) {
        const scrollDom = sideBarDom.getElementsByClassName('scroll-container')[0]
        if (scrollDom) {
          this.windowAvailableHeight = scrollDom.clientHeight
        }
      } else {
        this.windowAvailableHeight = document.getElementById('app').clientHeight
      }
